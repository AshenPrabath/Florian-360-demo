...
import { Save, Plus, Trash2, Home, Download, Copy, Image as ImageIcon, Crosshair, ChevronRight, ChevronDown, Folder, Camera, MapPin, Check, Sun, Compass, Star } from 'lucide-react';
...
const EditorPage = () => {
  ...
  const handleSetEntryPoint = (targetLocId, targetVpId) => {
    const updatedTourData = { ...tourData };
    // Clear all existing entry points, then set the selected one
    Object.values(updatedTourData).forEach(loc => {
      loc.viewpoints.forEach(vp => {
        delete vp.isEntryPoint;
      });
    });
    const vpIndex = updatedTourData[targetLocId].viewpoints.findIndex(v => v.id === targetVpId);
    updatedTourData[targetLocId].viewpoints[vpIndex].isEntryPoint = true;
    setTourData(updatedTourData);
  };
      ...
      <div className="w-72 bg-neutral-900 border-r border-white/10 flex flex-col">
            ...
            <div className="space-y-1">
              {Object.keys(tourData).map(locId => (
                <div key={locId} className="space-y-1">
                  <div className={`group flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all ${activeLocationId === locId ? 'bg-[#DCC5B7]/10 text-[#DCC5B7]' : 'hover:bg-white/5 text-white/70'
                    }`}>
                    <button
                      onClick={() => {
                        setActiveLocationId(locId);
                      ...
                      {tourData[locId].viewpoints.map(vp => (
                        <div key={vp.id} className="group/vp flex items-center justify-between px-3 py-1.5 rounded-md transition-all hover:bg-white/5">
                          <button
                            ...
                            className={`flex-grow flex items-center gap-2 text-xs transition-all ${activeViewpointId === vp.id ? 'text-[#DCC5B7] font-semibold' : 'text-white/40 hover:text-white font-normal'
                              }`}
                          >
                            <Camera size={12} className={activeViewpointId === vp.id ? 'text-[#DCC5B7]' : 'text-white/20'} />
                            {vp.name}
                            {vp.isEntryPoint && (
                              <Star size={10} className="text-yellow-400 fill-yellow-400 flex-shrink-0" title="Tour Entry Point" />
                            )}
                          </button>
                          <div className="flex items-center gap-0.5 opacity-0 group-hover/vp:opacity-100 transition-all">
                            <button
                              onClick={(e) => { e.stopPropagation(); handleSetEntryPoint(locId, vp.id); }}
                              className={`p-1 rounded transition-all ${vp.isEntryPoint
                                  ? 'text-yellow-400'
                                  : 'text-white/30 hover:text-yellow-400'
                                }`}
                              title={vp.isEntryPoint ? 'Tour Entry Point' : 'Set as Entry Point'}
                            >
                              <Star size={11} className={vp.isEntryPoint ? 'fill-yellow-400' : ''} />
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleDeleteViewpoint(locId, vp.id); }}
                              className="p-1 hover:bg-red-500/20 rounded text-red-400/60 transition-all"
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          <button
            ...
            disabled={isSaving}
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all ${saveSuccess
      ...
      <div className="flex-grow relative bg-black">
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 bg-neutral-900/80 backdrop-blur-xl border border-white/10 px-4 py-2 rounded-full shadow-2xl">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium transition-all ${editMode === 'view' ? 'bg-white text-black' : 'hover:bg-white/10'
              }`}
          >
            <Home size={14} />
            View Mode
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium transition-all ${editMode === 'rotate' ? 'bg-[#DCC5B7] text-black shadow-lg scale-105' : 'hover:bg-white/10'
              }`}
          >
            <Compass size={14} />
            Set Orientation
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium transition-all ${editMode === 'add' ? 'bg-[#DCC5B7] text-black' : 'hover:bg-white/10'
              }`}
          >
            <Plus size={14} />
            Add Hotspot
